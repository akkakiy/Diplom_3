...
# кнопка для закрытия окна с подтверждением заказа
    MODAL_CLOSE_BUTTON = (By.XPATH, ".//section[@class='Modal_modal_opened__3ISw4 Modal_modal__P3_V5']//button[@type='button']")
# корзина заказов
...
# Текст "Готовы:"
    ORDER_DONE = (By.XPATH, "//p[contains(text(),'Готовы:')]")
# Текст "В работе:"
    ORDER_IN_WORK = (By.XPATH, "//p[contains(text(),'В работе:')]")
# Текст "Выполнено за все время:"
    TOTAL_ODER_ALL_TIME = (By.XPATH, '//p[text()="Выполнено за сегодня:"]/following-sibling::p')
# Текст "Выполнено за сегодня:"
    DAILY_ORDER = (By.XPATH, "//p[text()='Выполнено за сегодня:']/following-sibling::p")
# Текст "Cостав:" в окне с подробностями заказа
